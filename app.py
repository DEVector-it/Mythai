...
from flask import Flask, Response, request, stream_with_context, session, jsonify, redirect, url_for, render_template
...
def view_shared_chat(chat_id):
    ...
    for msg in chat['messages']:
        sender = html.escape("You:" if msg['sender'] == 'user' else "Myth AI:")
        content = html.escape(msg['content']).replace('\n', '<br>')
        messages_html += f"<p><b>{sender}</b> {content}</p><hr>"
